...
import { PaymentRequestDetailEntity } from './paymentRequestDetail.entity';
import { MediaEntity } from './media.entity';

@Entity({ name: 'payment_request_list' })
export class PaymentRequestListEntity extends AbstractEntity {
    ...
    @Column({ type: 'enum', enum: PAYMENT_REQUEST_LIST_STATUS, default: PAYMENT_REQUEST_LIST_STATUS.DRAFT })
    status: PAYMENT_REQUEST_LIST_STATUS;
    ...
    @OneToMany(() => PaymentRequestDetailEntity, (entity) => entity.paymentRequestList, { createForeignKeyConstraints: false })
    detail: Relation<PaymentRequestDetailEntity>[];

    @ManyToMany(() => MediaEntity, { createForeignKeyConstraints: false })
    @JoinTable({
        name: 'payment_request_lists_attachments',
