...
import { LeavingLateEarlyEvent } from './events/leaving-late-early.event';
import { UserStorage } from '~/common/storages/user.storage';
import { EventEmitter2 } from '@nestjs/event-emitter';
import { LeavingLateEarlyEntity } from '~/database/typeorm/entities/leavingLateEarly.entity';
import { APPROVAL_ACTION, LEAVING_LATE_EARLY_STATUS } from '~/common/enums/enum';
...
@Injectable()
export class LeavingLateEarlyService {
    ...
    private emitEvent(event: string, data: { id: number }) {
        const eventObj = new LeavingLateEarlyEvent();
        eventObj.id = data.id;
        eventObj.senderId = UserStorage.getId();
        this.eventEmitter.emit(event, eventObj);
    }

    private async isStatusValid(data: { id: number; statuses: any[]; userId?: number; currentApproverId?: number }): Promise<LeavingLateEarlyEntity> {
        const entity = await this.database.leavingLateEarly.findOneBy({ id: data.id });
        if (!entity) throw new HttpException('Không tìm thấy đơn xin đi muộn, về sớm', 404);
        if (!data.statuses.includes(entity.status))
            throw new HttpException('Không thể chỉnh sửa đơn xin đi muộn, về sớm do trạng thái không hợp lệ', 400);
        if (data.userId && entity.createdById !== data.userId)
            throw new HttpException('Bạn không có quyền chỉnh sửa đơn xin đi muộn, về sớm này', 403);
    ...
    private async updateStatus(data: {
        id: number;
        from: LEAVING_LATE_EARLY_STATUS[];
        to: LEAVING_LATE_EARLY_STATUS;
        comment?: string;
        userId?: number;
        ...
        if (data.currentApproverId && data.nextApproverId && data.currentApproverId === data.nextApproverId)
            throw new HttpException('Bạn không thể chuyển đơn xin đi muộn, về sớm cho chính mình', 400);
        ...
        await this.database.leavingLateEarly.update(data.id, { status: data.to, currentApproverId: nextApprover });
        const step = await this.database.approvalHistory.getNextStep('leavingLateEarly', data.id);
    ...
    async create(createLeavingLateEarlyDto: CreateLeavingLateEarlyDto) {
        const isHigestLevel = await this.database.user.isHighestPosition(UserStorage.getId());
        const status = isHigestLevel ? LEAVING_LATE_EARLY_STATUS.APPROVED : LEAVING_LATE_EARLY_STATUS.DRAFT;
        const leavingLateEarly = await this.database.leavingLateEarly.save(
            this.database.leavingLateEarly.create({
                ...createLeavingLateEarlyDto,
                status,
                createdById: UserStorage.getId(),
            }),
        );

        this.emitEvent('leavingLateEarly.created', { id: leavingLateEarly.id });
        return leavingLateEarly;
    }

    async findAll(queries: FilterDto & { status: LEAVING_LATE_EARLY_STATUS; departmentId: string; month: number; year: number }) {
        return this.utilService.getList({
            repository: this.database.leavingLateEarly,
            queries,
            builderAdditional: async (builder) => {
                ...
                const conditions = await this.utilService.accessControlv2('leavingLateEarly');
    ...
    findOne(id: number) {
        const builder = this.database.leavingLateEarly.createQueryBuilder('entity');
        builder.leftJoinAndSelect('entity.createdBy', 'createdBy');
        builder.leftJoinAndSelect('createdBy.department', 'cbDepartment');
    ...
    async update(id: number, updateLeavingLateEarlyDto: UpdateLeavingLateEarlyDto) {
        await this.isStatusValid({
            id,
            statuses: [LEAVING_LATE_EARLY_STATUS.DRAFT, LEAVING_LATE_EARLY_STATUS.REJECTED],
            userId: UserStorage.getId(),
        });

        return this.database.leavingLateEarly.update(id, {
            ...updateLeavingLateEarlyDto,
            updatedById: UserStorage.getId(),
            status: LEAVING_LATE_EARLY_STATUS.DRAFT,
        });
    }
    ...
    async remove(id: number) {
        ...
        return this.database.leavingLateEarly.delete(id);
    }

    async approve(id: number) {
        await this.updateStatus({
            id,
            from: [LEAVING_LATE_EARLY_STATUS.IN_PROGRESS],
            to: LEAVING_LATE_EARLY_STATUS.APPROVED,
            currentApproverId: UserStorage.getId(),
            action: APPROVAL_ACTION.APPROVE,
        });

        this.emitEvent('leavingLateEarly.approved', { id });

        return { message: 'Đã duyệt đơn xin đi muộn, về sớm', data: { id } };
    }
    ...
    async reject(id: number, comment: string) {
        await this.updateStatus({
            id,
            from: [LEAVING_LATE_EARLY_STATUS.IN_PROGRESS],
            to: LEAVING_LATE_EARLY_STATUS.REJECTED,
            currentApproverId: UserStorage.getId(),
            action: APPROVAL_ACTION.REJECT,
            comment,
        });

        this.emitEvent('leavingLateEarly.rejected', { id });

        return { message: 'Đã từ chối đơn xin đi muộn, về sớm', data: { id } };
    }

    async forward(id: number, dto: NextApproverDto) {
        await this.updateStatus({
            id,
            from: [LEAVING_LATE_EARLY_STATUS.DRAFT, LEAVING_LATE_EARLY_STATUS.IN_PROGRESS],
            to: LEAVING_LATE_EARLY_STATUS.IN_PROGRESS,
        ...
        this.emitEvent('leavingLateEarly.forwarded', { id });

        return { message: 'Đã chuyển đơn xin đi muộn, về sớm', data: { id } };
    }
}
